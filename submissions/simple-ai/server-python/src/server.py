...
app.mount("/api/sites", StaticFiles(directory=str(SITES_DIR), html=True), name="api_sites")

# Production: serve the built SPA at /
DIST_DIR = Path(BASE_DIR).parent / "dist"
if DIST_DIR.exists():
    app.mount("/", StaticFiles(directory=str(DIST_DIR), html=True), name="spa")
    print(f"[SERVER] serving SPA from {DIST_DIR}")
else:
    print(f"[SERVER] no SPA build at {DIST_DIR} — running API-only (use vite for frontend)")
